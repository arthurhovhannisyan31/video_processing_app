...
export const InteractiveDotGrid: FC<InteractiveDotGridProps> = ({
  colors,
  children,
} = {}) => {
  ...
  useEffect(() => {
    // biome-ignore lint/style/noNonNullAssertion: Reference presence asserted
    ...
    function frame() {
      ...
      for (const d of dots) {
        ...
        const tgt = dist2 < r2 ? (1 - Math.sqrt(dist2) / RADIUS) ** 1.5 : 0;
  ...
  const bg = colors?.background ?? (isDark ? "#110F0C" : "#FFFFFF");
